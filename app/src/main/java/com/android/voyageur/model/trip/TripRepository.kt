...
interface TripRepository {
  ...
  fun init(onSuccess: () -> Unit)
     ...
     * @param onSuccess Callback that is triggered with the list of trips if the operation is successful.
  ...
  fun getFeed(
    ...
    /**
     * Listens for updates to trips for a specific user. This can be used for real-time updates or changes.
     *
     * @param userId The ID of the user whose trip updates are to be listened to.
     * @param onSuccess Callback that is triggered with the list of updated trips if the operation is successful.
     * @param onFailure Callback that is triggered if the operation fails, with the exception details.
     *
     * @return A `ListenerRegistration` object that can be used to unregister the listener when no longer needed.
     */
  fun listenForTripUpdates(
     ...
     * @param onSuccess Callback that is triggered with the `Trip` object if the operation is successful.
     * @param onFailure Callback that is triggered if the operation fails, with the exception details.
     */
  fun getTripById(
      tripId: String,
