...
class TripRepositoryFirebase(private val db: FirebaseFirestore) : TripRepository {
  ...
  /**
   * Fetches a list of trips that are discoverable and do not contain the user in their
   * participants. This method is used to populate the feed of trips that the user can join.
   *
        ...
        .addOnSuccessListener { result ->
          val trips =
              result
                  ...
                  .filter { it.participants.contains(userId).not() }
          onSuccess(trips)
        }
        .addOnFailureListener { exception ->
