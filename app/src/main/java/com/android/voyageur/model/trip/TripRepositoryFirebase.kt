...
/**
 * Implementation of [TripRepository] that interacts with Firebase Firestore to manage trips. This
 * class provides methods for creating, reading, updating, and deleting trips, as well as fetching
 * and listening for trip updates.
 */
class TripRepositoryFirebase(private val db: FirebaseFirestore) : TripRepository {
  ...
  /**
   * Initializes the repository by adding an authentication state listener to FirebaseAuth. If a
   * user is logged in, the onSuccess callback is invoked.
   *
   ...
   * @param onSuccess The callback to be invoked with the list of trips if the operation is
   *   successful.
   ...
   * @param onSuccess The callback to be invoked with the `Trip` object if the operation is
   *   successful.
   * @param onFailure The callback to be invoked if the operation fails, with the exception details.
   */
