...
class UserRepositoryFirebaseTest {
  ...
  fun testListenToUser_success() {
    ...
    `when`(mockDocumentReference.addSnapshotListener(any<EventListener<DocumentSnapshot>>())).thenAnswer { invocation ->
  ...
  fun testListenToUser_failure_exception() {
    // Arrange
    val userId = "1"
    val exception = FirebaseFirestoreException("Test exception", FirebaseFirestoreException.Code.UNKNOWN)
    val mockListenerRegistration = mock(ListenerRegistration::class.java)
